    ...
    private static List<String> clearanceNotes(Planner.Result r) {
        ...
        if (insideOwn > 0) {
            out.add(0, "У " + insideOwn + " точек подключения точка лежит внутри контура существующего ОКС "
                    + "(restriction_type = oks). Контур принят зданием самого ОКС: зазор до него не применяется, "
                    + "труба начинается в точке подключения и выходит из контура. Для остальных контуров зазор действует.");
        }
        return out;
    }
